...
apt-get remove -y cloudera-manager-server
/usr/sbin/service cloudera-scm-agent hard_stop
apt-get purge -y 'cloudera-manager-*'

apt-get remove -y cloudera-manager-server-db-2 postgresql
...
rm -Rf /opt/cloudera /etc/hadoop /etc/cloudera-scm-agent /etc/hive /var/run/hdfs-* /var/lib/hive /var/lib/alternatives/hive-conf /etc/alternatives/hive-conf /var/run/hbase /var/log/impalad /var/run/impala /var/lib/impala /var/run/sqoop2 /var/run/solr /var/run/zookeeper /var/run/hadoop-httpfs /var/lib/alternatives/hadoop-conf /etc/alternatives/hadoop-conf
